...
async function generateQuiz() {
  ...
  const genAI = new GoogleGenerativeAI("AIzaSyBLrcv03isef5CgS0OUVEv7JbS1YcQgotw");
  const model = genAI.getGenerativeModel({ model: MODEL_NAME });

  const prompt = "you are a quiz master.generate 10 random questions in computer science with 4 multiple choice answers.also provide the answer seperatly the response should be in the wollowung json format:{ \"questions\":[{id:0,\"question\":\"\",\"option\":[],\"answer\":\"\"},...]}";
...
exports.getQuiz = async (req, res) => {
  ...
  console.log("ddddddddd")
    ...
    if (user.Quiz.QuizList.length > 0 && user.Quiz.QuizList[user.Quiz.QuizList.length - 1].date ===today){
